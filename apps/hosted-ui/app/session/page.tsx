...
async function fetchCustomerInfo(customerId: string): Promise<CustomerInfo | null> {
  try {
    ...
    if (transactionData && selectedTransactionId) {
      ...
      if (terminalStatuses.includes(transactionData.status)) {
        setCompletionData({
          ...
          outcome: transactionData.status === 'CANCELLED' ? 'cancelled' : 'success',
        });
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">
            {customerLoading ? 'Loading transaction details…' : 'Loading session…'}
          </p>
        </div>
      </div>
    );
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          ...
          {currentView === 'transaction-monitor' && selectedTransactionId && (
            <div>
              <BackButton onClick={() => setCurrentView('wallet')} label="Back to Wallet" />
              <StatusMonitor
                transactionId={selectedTransactionId}
                apiKey={apiKey}
          ...
          {currentView === 'dispute-form' && selectedTransactionId && (
            <div>
              <BackButton onClick={() => setCurrentView('transaction-monitor')} label="Back to Transaction" />
              <DisputeForm
                transactionId={selectedTransactionId}
          ...
          {currentView === 'dispute-thread' && currentDispute && selectedTransactionId && (
            <div>
              <BackButton onClick={() => setCurrentView('transaction-monitor')} label="Back to Transaction" />
