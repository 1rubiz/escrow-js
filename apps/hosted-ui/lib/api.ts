...
export interface TransactionsResponse {
  data: PaylukEscrow[]
  pagination: { count: number, pages: number, isLastPage: boolean, nextPage: string | null, previousPage: string | null }
  report: {
 ...
 */
export async function validateSessionToken(token: string): Promise<ApiResponse<{
  customerId: string;
 ...
 */
export async function getWallet(customerId: string, apiKey: string): Promise<ApiResponse<{
 ...
 * Get wallet information for a customer
 */
export async function getTransactions(customerId: string, type: "sales" | "buy", apiKey: string): Promise<ApiResponse<TransactionsResponse>> {
  try {
    const response = await fetch(`/api/payluk/transactions/get/${customerId}/${type}`, {
      method: 'GET',
      headers: {
        'x-api-key': apiKey,
        'x-nonce': `nonce_${Date.now()}_${Math.random().toString(36).substring(2, 15)}`,
...
export async function markAsDelivered(
  ...
  try {
    const response = await fetch(`/api/payluk/transactions/${transactionId}/deliver`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': apiKey,
...
export async function releasePayment(
  ...
  try {
    const response = await fetch(`/api/payluk/transactions/${transactionId}/release`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': apiKey,
